...
export type ShootingStarsGridTheme = {
  ...
  starColor?: string;
  starShadow?: string;
  shootingStarFrom?: string;
  shootingStarTo?: string;
...
const DEFAULT_THEME: Required<ShootingStarsGridTheme> = {
  gridLineColor: "rgba(255,255,255,0.075)",
  overlayColor: "rgba(7,9,15,0.22) 52%, rgba(7,9,15,0.96) 100%",
  glowPrimary: "rgba(34,211,238,0.22)",
  glowSecondary: "rgba(168,85,247,0.16)",
  starColor: "rgba(255,255,255,0.55)",
  starShadow: "rgba(255,255,255,0.55)",
  shootingStarFrom: "rgba(8,145,178,0.14)",
  shootingStarTo: "rgba(255,255,255,0.96)",
  shootingStarGlow: "rgba(6,182,212,0.46), 0 0 28px rgba(148,163,184,0.18)",
};
...
function createStaticStars(count: number): StaticStar[] {
  return Array.from({ length: count }, (_, index) => ({
    x: Math.round(seeded(index, 1) * 100 * 1000) / 1000,
    y: Math.round(seeded(index, 2) * 100 * 1000) / 1000,
    size: Math.round((1 + seeded(index, 3) * 2.4) * 100) / 100,
    opacity: Math.round((0.16 + seeded(index, 4) * 0.44) * 100) / 100,
    delay: Math.round(seeded(index, 5) * 4 * 100) / 100,
    duration: Math.round((2.4 + seeded(index, 6) * 3.2) * 100) / 100,
  }));
}
...
function createShootingStars(count: number): ShootingStar[] {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      length: Math.round(86 + seeded(index, 15) * 132),
      delay: Math.round((seeded(index, 16) * 7 + index * 0.65) * 100) / 100,
      duration: Math.round((1.65 + seeded(index, 17) * 1.6) * 100) / 100,
      repeatDelay: Math.round((4.8 + seeded(index, 18) * 6.2) * 100) / 100,
      direction,
    };
  });
}
...
function GridRunner({
  ...
  speedScale: number;
  theme: Required<ShootingStarsGridTheme>;
}) {
  ...
  return (
    <motion.span
      ...
      style={{
        ...
        background: `linear-gradient(${gradientDirection}, transparent 0%, ${theme.shootingStarFrom} 18%, ${theme.shootingStarTo} 58%, transparent 100%)`,
        boxShadow: `0 0 16px ${theme.shootingStarGlow.split(",")[0].trim()}, 0 0 28px ${theme.shootingStarGlow.split(",")[1]?.trim() || "rgba(148,163,184,0.18)"}`,
      }}
  ...
  const effects = (
    <>
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle_at_center,transparent 0%,${theme.overlayColor})`,
        }}
      />
      ...
      {glow && (
        <motion.div
          ...
          style={{
            background: `radial-gradient(circle_at_var(--shooting-stars-glow-x)_var(--shooting-stars-glow-y),${theme.glowPrimary},transparent_34%),radial-gradient(circle_at_72%_72%,${theme.glowSecondary},transparent_30%)`,
          }}
          ...
          {staticStars.map((star, index) => (
            <motion.span
              ...
              style={{
                ...
                boxShadow: `0 0 12px ${theme.starShadow}`,
              }}
              animate={
                shouldAnimate
